...
func init() {
	...
	f.StringVar(&path, "show-asset", "", "show specified asset")
	f.BoolVar(&list, "list-asset", false, "list asset")
	f.BoolVar(&restore, "restore-asset", false, "restore asset")
	f.StringVar(&restorePath, "restore-path", ".", "restore asset to specified path")
...
func (e *embedding) list() error {
	return fs.WalkDir(assetFS, ".", func(path string, d fs.DirEntry, err error) error {
		if path == "." || path == ".." {
			return nil
		}
		if d.IsDir() {
...
func (e *embedding) show(path string) error {
	code, err := fs.ReadFile(assetFS, path)
	if err != nil {
		return err
	}
	e.write(string(code))
